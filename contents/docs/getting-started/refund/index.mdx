...
const response = await axios.post(
  "https://api.catopay.com/api/v1/refund/create",
  {
...
If the user's wallet has Secret enabled, add the `walletSecret` field to the payload.

**Request Payload (With Wallet Secret):**
...
const response = await axios.post(
  "https://api.catopay.com/api/v1/refund/create",
  {
    ...
    accountType: "PERSONAL_ACCOUNT", // Account type: PERSONAL_ACCOUNT, BUSINESS_ACCOUNT, etc.
    bankNumberOrAddress: "1234567890", // Bank account number or address
    walletSecret: "YourWalletSecret", // Wallet secret for enabled accounts
  },
  {
    headers: {
      "Content-Type": "application/json",
