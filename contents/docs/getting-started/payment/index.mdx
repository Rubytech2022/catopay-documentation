...
const response = await axios.post("https://api.catopay.com/api/v1/payment/create", {
    ...
    suggested_numbers: "1234567890,0987654321"  // Comma-separated numbers for the transaction page
}, {
    headers: {
        "Content-Type": "application/json",
...
- You can provide one or more suggested numbers using the `suggested_numbers` parameter in the request.
...
- If you use `suggested_numbers`, they will be shown on the transaction page, allowing users to choose from them or enter a number manually if not provided.
